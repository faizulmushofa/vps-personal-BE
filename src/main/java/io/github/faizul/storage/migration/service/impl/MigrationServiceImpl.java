package io.github.faizul.storage.migration.service.impl;
...
public class MigrationServiceImpl implements MigrationService {
    ...
    @Override
    public Mono<Void> cancelTaskByBatchIdAndFileId(UUID batchId, String fileId) {
        ...
        return resolvedFileIdMono
                .flatMap(resolvedFileId -> currentUserContext.getUserId()
                        .flatMap(userId -> migrationTaskRepository.findByBatchId(batchId)
                                .filter(task -> task.getFileId().equals(resolvedFileId))
                                .next()
                                .switchIfEmpty(Mono.error(new NoSuchElementException("Tugas migrasi tidak ditemukan.")))
                                .flatMap(task -> {
                                    if (!task.getUserId().equals(userId)) {
