...
import io.github.faizul.storage.share.model.FileShared;
import io.github.faizul.storage.share.repository.FileSharedRepository;
import io.github.faizul.storage.share.local.service.StorageNodeShareService;
...
public class StorageNodeShareServiceImpl implements StorageNodeShareService {
                        ...
                        .flatMap(file -> fileSharedRepository.findByFileId(file.getId())
                                .flatMap(shared -> {
                                    if (Boolean.TRUE.equals(shared.getIsPublic())) {
                                        return Mono.just(new SharedByMeResponse(
                                                shared.getId(),
                                                file.getId(),
                                                file.getOriginalFileName(),
                                                file.getSize(),
                                        ...
                                        return userRepository.findById(shared.getUserId())
                                                .map(targetUser -> new SharedByMeResponse(
                                                        shared.getId(),
                                                        file.getId(),
                                                        file.getOriginalFileName(),
                                                        file.getSize(),
                                                ...
                                                .defaultIfEmpty(new SharedByMeResponse(
                                                        shared.getId(),
                                                        file.getId(),
                                                        file.getOriginalFileName(),
                                                        file.getSize(),
    ...
    @Override
    public Mono<ShareFileResponse> shareFileWithLog(String fileId, ShareFileRequest request, org.springframework.web.server.ServerWebExchange exchange) {
        UUID fileUUID = UUID.fromString(fileId);
        return currentUserContext.getUserId()
                .flatMap(userId -> fileRepository.findById(fileUUID)
                        .flatMap(file -> shareFile(fileId, request)
    ...
    @Override
    public Mono<Void> unshareFileWithLog(String fileId, Long targetUserId, org.springframework.web.server.ServerWebExchange exchange) {
        UUID fileUUID = UUID.fromString(fileId);
        return currentUserContext.getUserId()
                .flatMap(adminId -> fileRepository.findById(fileUUID)
                        .flatMap(file -> unshareFile(fileId, targetUserId)
                                .then(userActivityService.log(adminId, "UNSHARE_FILE", "Membatalkan share berkas " + file.getOriginalFileName() + " untuk user ID: " + targetUserId, exchange))
                        )
                )
                .then();
    }
    ...
    @Override
    public Mono<Void> unshareFileByIdWithLog(Long shareId, org.springframework.web.server.ServerWebExchange exchange) {
        return currentUserContext.getUserId()
                .flatMap(userId -> unshareFile(shareId)
    ...
    @Override
    public Flux<byte[]> downloadPublicFileWithLog(String shareToken, org.springframework.web.server.ServerWebExchange exchange) {
        return getPublicFileInfo(shareToken)
                .flatMapMany(file -> userActivityService.log(null, "DOWNLOAD_SHARED_PUBLIC", "Mengunduh berkas publik: " + file.originalFileName() + " dengan token: " + shareToken, exchange)
                        .thenMany(downloadPublicFile(shareToken)));
    }
