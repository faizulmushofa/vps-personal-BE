package io.github.faizul.storage.share.gdrive.service.impl;
...
public class GoogleDriveShareServiceImpl implements GoogleDriveShareService {
    ...
    public Mono<ShareFileResponse> shareFileWithLog(String fileId, ShareFileRequest request, org.springframework.web.server.ServerWebExchange exchange) {
        return currentUserContext.getUserId()
                .flatMap(userId -> shareFile(fileId, request)
                        .flatMap(response -> {
    ...
    public Mono<Void> unshareFileWithLog(String fileId, Long targetUserId, org.springframework.web.server.ServerWebExchange exchange) {
        return currentUserContext.getUserId()
                .flatMap(adminId -> unshareFile(fileId, targetUserId)
                        .then(Mono.defer(() -> {
                            Mono<String> getFileNameMono;
                            try {
                                UUID uuid = UUID.fromString(fileId);
                            ...
                            return getFileNameMono.flatMap(fileName -> 
                                userActivityService.log(adminId, "UNSHARE_FILE_GD", "Membatalkan share berkas Google Drive " + fileName + " untuk user ID: " + targetUserId, exchange)
                            );
                        }))
                )
                .then();
    ...
    public Flux<byte[]> downloadPublicFileWithLog(String shareToken, org.springframework.web.server.ServerWebExchange exchange) {
        return getPublicFileInfo(shareToken)
                .flatMapMany(file -> userActivityService.log(null, "DOWNLOAD_SHARED_PUBLIC_GD", "Mengunduh berkas publik Google Drive: " + file.originalFileName() + " dengan token: " + shareToken, exchange)
